...
const AIAssistant = () => {
  ...
  const sendMessage = async (text) => {
    ...
    // Always show the response (success or error fallback)
    if (res.data) {
    ...
    <div className="flex flex-col h-full" style={{ height: 'calc(100vh - 80px - 48px)' }}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 mt-1">Your autonomous business intelligence co-pilot.</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={handleReset}
            title="Clear conversation"
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-slate-400 hover:text-white bg-slate-800/60 border border-slate-700/50 hover:border-slate-600 rounded-full transition-all"
          >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <div className={`w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center ${
                msg.role === 'assistant'
                  ? 'bg-gradient-to-br from-brand-400 to-indigo-600 shadow-lg shadow-brand-500/20'
                  : 'bg-slate-700'
              }`}>
                {msg.role === 'assistant'
          ...
          {isThinking && (
            <div className="flex gap-4">
              <div className="w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center bg-gradient-to-br from-brand-400 to-indigo-600 shadow-lg shadow-brand-500/20">
                <Sparkles className="w-4 h-4 text-white" />
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              {SUGGESTED_PROMPTS.map((p) => (
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs px-3 py-2 bg-slate-800/60 border border-slate-700/50 rounded-lg text-slate-300 hover:border-brand-500/50 hover:text-brand-400 transition-all"
                >
        ...
        <div className="p-4 border-t border-slate-700/50">
          <div className="flex items-end gap-3">
            ...
            <button
              ...
              className="flex-shrink-0 w-11 h-11 flex items-center justify-center rounded-xl bg-brand-500 text-white shadow-lg shadow-brand-500/20 hover:bg-brand-600 disabled:opacity-40 disabled:cursor-not-allowed transition-all hover:-translate-y-0.5 active:translate-y-0"
            >
