...
const IntegrationCard = ({ name, description, logo, connected }) => (
  <div className="flex items-center justify-between p-4 bg-slate-800/40 border border-slate-700/50 rounded-xl">
    <div className="flex items-center gap-4">
      <div className="w-10 h-10 rounded-xl bg-slate-700/50 flex items-center justify-center text-lg">{logo}</div>
      <div>
...
const Settings = () => {
  ...
  const handleTestN8n = async () => {
    ...
    try {
      ...
      setN8nTestResult(res?.data || { success: false, message: 'No response from n8n test endpoint' });
    } catch (err) {
  ...
  const handleCopyN8nUrl = () => {
    const url = n8nStatus?.webhookUrl || 'http://localhost:5678/webhook/bizpilot-churn-retention';
    navigator.clipboard?.writeText(url);
    setCopiedN8nUrl(true);
    setTimeout(() => setCopiedN8nUrl(false), 2000);
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-5 pb-6 border-b border-slate-700/50">
              ...
              <div>
                ...
                <button className="mt-2 text-xs text-brand-400 hover:text-brand-300 transition-colors">Change avatar</button>
              </div>
            </div>
        ...
        return (
          <div className="space-y-5">
            <div className="p-4 bg-brand-500/5 border border-brand-500/20 rounded-xl">
              <p className="text-sm text-slate-300">🔒 Your account uses a secure authentication system. Change your password below.</p>
            </div>
            ...
            <div className="pt-2 border-t border-slate-700/50">
              <p className="text-sm font-semibold text-slate-200 mb-3">Two-Factor Authentication</p>
              <p className="text-sm text-slate-400 mb-3">Add an extra layer of security to your account by enabling 2FA.</p>
      ...
      case 'notifications':
        return (
          <div>
            <p className="text-sm text-slate-400 mb-6">Choose which alerts and reports the AI agent sends to you.</p>
          ...
          <div className="space-y-6">
            <div>
              <p className="text-sm text-slate-400 mb-4">
                Connect BizPilot AI with your automation workflows and third-party platforms.
              </p>
            </div>

            {/* n8n Churn Automation Workflow Card */}
            <div className="p-5 rounded-2xl border border-indigo-500/40 bg-slate-900 shadow-xl space-y-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-bold text-white">n8n Churn Win-Back Automation</h3>
                      <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                        Webhook Trigger
                      </span>
                    </div>
                    <p className="text-xs text-slate-400 mt-1">
                      Autonomous event pipeline dispatching rich RFM churn profiles and personalized incentives.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  >
                    <RefreshCw className={`w-3.5 h-3.5 ${isTestingN8n ? 'animate-spin text-brand-400' : ''}`} />
                    <span>{isTestingN8n ? 'Pinging n8n...' : 'Test Webhook'}</span>
                  </button>
                  <a
              ...
              <div className="p-3.5 bg-slate-950 rounded-xl border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="overflow-hidden">
                  <p className="text-[11px] uppercase tracking-wider font-semibold text-slate-400 mb-0.5">Target Webhook Endpoint</p>
                  <code className="text-xs text-brand-300 font-mono break-all">
                    {n8nStatus?.webhookUrl || 'http://localhost:5678/webhook/bizpilot-churn-retention'}
                  </code>
                </div>
                <button
                ...
                >
                  ...
                  <span>{copiedN8nUrl ? 'Copied!' : 'Copy URL'}</span>
                </button>
              </div>
                ...
                <div className={`p-3 rounded-xl border text-xs flex items-start gap-2.5 ${
                  ...
                  <div className="space-y-1">
                    <p className="font-semibold">
                      {n8nTestResult.success ? 'n8n Webhook Active & Verified' : 'n8n Connectivity Diagnostic'}
                    </p>
                    <p className="text-[11px] opacity-90">{n8nTestResult.message}</p>
                    {n8nTestResult.error && (
                      <p className="font-mono text-[10px] text-slate-400 mt-1">{n8nTestResult.error}</p>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="pt-3 border-t border-slate-800 space-y-2.5 text-xs text-slate-400">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800 space-y-1">
                    <span className="font-bold text-brand-400">1. Import Workflow</span>
                    <p className="text-[11px]">Click "Download Workflow JSON" and import it directly into your n8n workspace.</p>
                  </div>
                  <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800 space-y-1">
                    <span className="font-bold text-brand-400">2. Configure SMTP</span>
                    <p className="text-[11px]">Set your Gmail, SendGrid, Mailgun, or corporate SMTP credentials on the Email node.</p>
                  </div>
                  <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800 space-y-1">
                    <span className="font-bold text-brand-400">3. Activate Workflow</span>
                    <p className="text-[11px]">Toggle the workflow "Active" in n8n. BizPilot AI sends retention emails automatically.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-2">
              <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-400">Connected Services</h4>
              <IntegrationCard name="Shopify" description="Sync orders and inventory automatically" logo="🛍️" connected={true} />
              <IntegrationCard name="Stripe" description="Pull real-time payment and revenue data" logo="💳" connected={true} />
              <IntegrationCard name="Mailchimp" description="Trigger email campaigns based on AI insights" logo="📧" connected={false} />
              <IntegrationCard name="QuickBooks" description="Sync financial records and invoices" logo="📊" connected={false} />
              <IntegrationCard name="Slack" description="Receive AI alerts directly in your Slack workspace" logo="💬" connected={false} />
            </div>
          </div>
        );
          ...
          <div className="space-y-6">
            ...
            <div className={`p-5 rounded-2xl border transition-all ${
              ...
              {dbStatus?.error && (
                <div className="mt-4 p-3 bg-slate-900/80 rounded-xl border border-slate-800 text-xs text-slate-400 font-mono flex items-start gap-2">
                  ...
                  <div>
                    <span className="text-rose-400 font-semibold">Diagnostic: </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-slate-800/40 border border-slate-700/50 rounded-xl">
                <p className="text-xs text-slate-400">Target Database URI</p>
                <div className="flex items-center justify-between mt-1">
                  <code className="text-sm text-brand-300 font-mono truncate">
                    {dbStatus?.uri || 'mongodb://localhost:27017/bizpilot'}
                  </code>
                </div>
              </div>
              <div className="p-4 bg-slate-800/40 border border-slate-700/50 rounded-xl">
                <p className="text-xs text-slate-400">Operating Mode</p>
            ...
            <div className="p-5 bg-slate-800/20 border border-slate-700/40 rounded-2xl space-y-4">
              ...
              <div className="space-y-3 text-xs text-slate-400 leading-relaxed">
                <div className="p-3 bg-slate-900/60 rounded-xl border border-slate-800">
                  <p className="font-semibold text-slate-300 mb-1">Option A: Start Local MongoDB (Windows)</p>
                  <p>If MongoDB Community Server is installed on this PC, start the service in PowerShell:</p>
                  <pre className="mt-2 p-2 bg-black/40 rounded border border-slate-800 text-brand-300 font-mono">net start MongoDB</pre>
                </div>

                <div className="p-3 bg-slate-900/60 rounded-xl border border-slate-800">
                  <p className="font-semibold text-slate-300 mb-1">Option B: Free MongoDB Atlas Cloud Database</p>
                  <p>1. Create a free cluster on <span className="text-brand-400 font-medium">mongodb.com/atlas</span>.</p>
                  <p>2. Copy your connection string and add it to <code className="text-white">backend/.env</code>:</p>
                  <pre className="mt-2 p-2 bg-black/40 rounded border border-slate-800 text-brand-300 font-mono">MONGODB_URI=mongodb+srv://&lt;username&gt;:&lt;password&gt;@cluster0.mongodb.net/bizpilot</pre>
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-1">Manage your account, business, and preferences.</p>
      </div>
