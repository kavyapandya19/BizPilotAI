...
const Dashboard = () => {
  ...
  const getColorForTitle = (title) => {
    if (title.includes('Revenue')) return { text: 'text-brand-400', bg: 'bg-brand-400/10' };
    if (title.includes('Customers')) return { text: 'text-indigo-400', bg: 'bg-indigo-400/10' };
    if (title.includes('Orders')) return { text: 'text-orange-400', bg: 'bg-orange-400/10' };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Here is what's happening with your business today.</p>
        </div>
        <button 
        ...
        {kpis.map((kpi, index) => {
          ...
          return (
            <div key={index} className="glass-card p-6">
              ...
              <div className="mt-4">
                <h3 className="text-slate-400 font-medium">{kpi.title}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-6 flex flex-col">
          <h2 className="text-lg font-semibold text-white mb-6">AI Agent Insights</h2>
            ...
            {insights.map((insight) => (
              <div 
                ...
                className={`p-4 rounded-xl border ${
                  insight.type === 'alert' 
                    ? 'bg-gradient-to-r from-brand-500/10 to-transparent border-brand-500/20' 
                    : 'bg-slate-800/50 border-slate-700/50'
                }`}
              >
                <div className="flex items-center gap-2 mb-2">
                  <div className={`w-2 h-2 rounded-full ${insight.type === 'alert' ? 'bg-brand-400' : 'bg-emerald-400'}`}></div>
                  <span className={`text-sm font-medium ${insight.type === 'alert' ? 'text-brand-400' : 'text-emerald-400'}`}>
                    {insight.title}
                  </span>
                </div>
